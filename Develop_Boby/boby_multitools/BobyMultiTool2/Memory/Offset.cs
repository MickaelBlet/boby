...
namespace MemoryLib
{
    ...
    public class Offset
    {
        public static Dictionary<string, string> offsetList = new Dictionary<string, string>();
        ...
        public class Character
        {
            ...
            public Character(int Base, IntPtr hanble)
            {
                Name = Memory.ReadString(hanble, Base + Offset.Player.Name, 30 * 2, true);
                if (Name == string.Empty)
                    return;
                Guild = Memory.ReadString(hanble, Base + Offset.Player.Guild, 30 * 2, true);
                Lvl = Memory.ReadByte(hanble, Base + Offset.Player.Lvl).ToString();
                Class = ((Aion_Game.eClass)Memory.ReadByte(hanble, Base + Offset.Player.Class)).ToString();
                Race = ((Aion_Game.eRace)Memory.ReadByte(hanble, Base + Offset.Player.Race)).ToString();
                Server = ((Aion_Game.eServeur)Memory.ReadByte(hanble, Base + Offset.Game.Server)).ToString();
            }

            public void Update(int Base, IntPtr hanble)
            {
                Name = Memory.ReadString(hanble, Base + Offset.Player.Name, 30 * 2, true);
                if (Name == string.Empty)
                    return;
        ...
        public static void SendCharacter()
        {
            if (lastSend < DateTime.Now)
            {
                new Thread(() =>
                {
                    string encrypt = "";
                    foreach (var character in characterList.Values)
                    {
                        ...
                        encrypt += ';' + character.Guild;
                        encrypt += ';' + character.Lvl;
                        encrypt += ';' + character.Class;
                        encrypt += ';' + character.Race;
                        encrypt += ';' + character.Server + '.';
                    }
                ...
                lastSend = DateTime.Now.AddMinutes(15);
            }
        }
        ...
        public static bool Download(Window main)
        {
            ...
            var response = BobyMultitools.App.Offset;
        ...
        public static bool Loading(int Base, string version, IntPtr hanble)
        {
            ...
            var b64decode = Base64Decode(offsetList[version]);

            //MessageBox.Show(b64decode);

            var hashkey = int.Parse(b64decode.Substring(0, 1));
            hashkey = hashkey * 10 + int.Parse(b64decode.Substring(b64decode.Length - 1, 1));
            var hashlist = b64decode.Substring(1, b64decode.Length - 2);
            ...
            try
            {
                ...
                Ability.Maintain = offset["Ability.Maintain"];
                Ability.Lvl = offset["Ability.Lvl"];

                // Dialog
                Dialog.Array = offset["Dialog.Array"];
                Dialog.ArraySize = offset["Dialog.ArraySize"];
                Dialog.Validate = offset["Dialog.Validate"];
                ...
                Cube.Node = offset["Cube.Node"];
                Cube.Id = offset["Cube.Id"];
                Cube.Buffer = offset["Cube.Buffer"];
                Cube.BufferSize = offset["Cube.BufferSize"];
                Cube.CooldownTime = offset["Cube.CooldownTime"];
                Cube.CooldownTimeRemaining = offset["Cube.CooldownTimeRemaining"];
                Cube.Type = offset["Cube.Type"];
                Cube.Slot = offset["Cube.Slot"];
                ...
                Entity.Z_Colision = offset["Entity.Z_Colision"];
                ...
                if (!characterList.ContainsKey(Base))
                    characterList[Base] = new Character(Base, hanble);
                else
                    characterList[Base].Update(Base, hanble);
            }
            catch
            {
                MessageBox.Show("Offsets Not Found", "Error");
                return false;
            }
        ...
        public class Entity
        {
            ...
            public static long Atk_Speed = 0;
            public static long Mov_Speed = 0;
            public static long Z_Colision = 0;
