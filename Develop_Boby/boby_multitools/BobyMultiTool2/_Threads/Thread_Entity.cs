...
namespace _Threads
{
    public class Thread_Entity
    {
        ...
        public static Dictionary<long, Aion_Game.Entity> eListNew = new Dictionary<long, Aion_Game.Entity>();
        public static Dictionary<long, Entity> DicCopy = new Dictionary<long, Entity>(eList);
        public Dictionary<long, Aion_Game.Entity> DicCopyNew = new Dictionary<long, Aion_Game.Entity>(eListNew);
        HashSet<long> found = null;
        HashSet<long> found2 = null;
        ...
        void ThreadStartEntity()
        {
            if (Game.Pid != 0 && Game.Base != 0)
            {
                ...
                try
                {
                    //tWhere = SplMemory.ReadWchar(Game.Base + Offset.Game.Where, 60);
                    ePlayer.Update();
                    //Update();
                    DicCopy = new Dictionary<long, Entity>(eList);
                    //DicCopyNew = new Dictionary<long, Aion_Game.Entity>(eListNew);
        ...
        public void Update()
        {
            ...
            found2 = new HashSet<long>();

            //long entityMap = SplMemory.ReadLong(Game.Base + (long)Offset.EntityList.Pointer);
            ...
            /*foreach (var entity in eListNew.Values)
            {
                if (found.Contains(entity.Node))
                {
                    if (entity.Id != 0 || entity.Name != "" || entity.Type == Aion_Game.eType.Vehicle)
                        entity.Update();
                }
            }
        ...
        void TraverseNode(long nodePtr, int count = 0)
        {
            try
            {
                if (count > 5000 || nodePtr == 0 || nodePtr == stop || !found2.Add(nodePtr))
                    return;
                long entityPtr = SplMemory.ReadLong(nodePtr + 12);
                if (entityPtr == 0 || entityPtr == stop || !found.Add(entityPtr))
                    return;
                TraverseNode(SplMemory.ReadLong(nodePtr), ++count);
            }
            catch
            {
                //MessageBox.Show(e.ToString());
    ...
    public class Player
    {
        ...
        /*public class Entity
        {
            ...
            public void Update()
            {
                if (PtrEntity != 0 && PtrEntity != stop)
                {
                    try
                    {
                        ...
                        if (EntityLoc == 0 || EntityLoc == 0xCDCDCDCD)
                            return;
            ...
            public void Update2()
            {
                ...
                this.Nametolower = RemoveDiacritics(this.Name.ToLower());
            ...
            public void Update_img()
            {
                ...
                {
                    if (this.DistanceReal < 113 && Thread_Entity.in_Win_Main.in_Setting.in_Entity.Where != "" && this.Nametolower.Contains(RemoveDiacritics(Thread_Entity.in_Win_Main.in_Setting.in_Entity.Where.ToLower())))
                    {
                    ...
                    else if (tWhere != "" && this.Nametolower.Contains(RemoveDiacritics(tWhere.ToLower())))
                    {
            ...
            Icon Check_If_PNG()
            {
                if (Thread_Entity.in_Win_Main == null || Thread_Entity.in_Win_Main.in_Win_Radar == null || Thread_Entity.in_Win_Main.in_Win_Radar.in_Win_Radar_Setting == null)
                    return null;
