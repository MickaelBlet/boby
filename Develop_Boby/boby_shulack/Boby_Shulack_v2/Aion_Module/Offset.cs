...
namespace MemoryLib
{
    ...
    public class Offset
    {
        ...
        public static void Loading(int Base)
        {
            ...
            Window.List.Base = 0xEC88D0;
            Window.List.Size = 0x700;

            Window.List l = new Window.List();

            l.GetFullList(Base);

            Window.PartyDialog.FirstMemberID = 0x480;
            Window.PartyDialog.SizeMember = 0x7C;
            Window.ForceDialog.FirstMemberID = 0x488;
            Window.ForceDialog.SizeMember = 0x24;
        ...
        public static class Window
        {
            ...
            public class List
            {
                ...
                public void GetFullList(int Game_Base)
                {
                ...
                {
                    string name = SplMemory.ReadChar(Base + Window.Name, 30);
                    if (!Regex.IsMatch(name, @"^[a-z0-9_]+$"))
                    {
                        var nameAddress = SplMemory.ReadLong(Base + Window.Name);
                ...
                void SearchControl(long Base, string name)
                {
                    if (Base != 0 && Base != stop)
                    {
                        long search = SplMemory.ReadLong(Base + Window.Tree);
                        if (search != 0 && search != stop)
                        {
                            search = SplMemory.ReadLong(search);
                            if (search != 0 && search != stop)
                            {
                                byte i = 0;
                                while (SplMemory.ReadLong(search + 0x8) != 0 && SplMemory.ReadLong(search + 0x8) != stop)
                                {
